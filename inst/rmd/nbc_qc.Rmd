...
```{r, eval=AVAILABLE_POOLED_QC&&AVAILABLE_REFERENCE_QC}
compounds_with_mvs <- biocrates %>%
  filter(
    !!sym(COLUMN_SAMPLE_TYPE) %in%
      c(SAMPLE_TYPE_POOLED_QC, SAMPLE_TYPE_REFERENCE_QC)
  ) %>%
  group_by(Compound) %>%
  filter(na_percent(UQ(sym(PKG_ENV$CONCENTRATION))) > 0) %>%
  select(Compound) %>%
  distinct()
fig_width_mv_variable_bars <- max(5, nrow(compounds_with_mvs) * 0.11 + 0.45)
```

```{r qc_mv_comparison, results="asis", fig.height=4, fig.width=fig_width_mv_variable_bars, eval=AVAILABLE_POOLED_QC&&AVAILABLE_REFERENCE_QC}
...
qc_comp_plot <- plot_compound_na_variable_bars(
  ...
  target = PKG_ENV$CONCENTRATION,
  sample_type = c(SAMPLE_TYPE_POOLED_QC, SAMPLE_TYPE_REFERENCE_QC),
