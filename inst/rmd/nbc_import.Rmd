...
if (length(unique(conc_col_name)) == 1) {
  ...
  assign("CONCENTRATION", unique(conc_col_name), PKG_ENV)
} else {
  # Otherwise replace concentration column names by global name without units
  for (i in seq_len(num_batches)){
    names(imports[[i]])[conc_col_name[i]] <- PKG_ENV$CONCENTRATION
  }
  # And warn user
...
if (meas_type == "LC") {
  DATA_TYPES <- c(PKG_ENV$CONCENTRATION, AREA, INTENSITY, ITSD_AREA, ITSD_INTENSITY)
} else { # "FIA"
  DATA_TYPES <- c(PKG_ENV$CONCENTRATION, INTENSITY, ITSD_INTENSITY)
}
```
...
easy_datatable(duplicate_samples, caption = "Sample duplicates.")
CONTINUE <- FALSE
```
