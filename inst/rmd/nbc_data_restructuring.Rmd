...
```{r, results="asis"}
# Are there any SAMPLE_TYPE_REFERENCE_QC samples? More than one?
AVAILABLE_REFERENCE_QC <- SAMPLE_TYPE_REFERENCE_QC %in% biocrates$Sample.Type
ENOUGH_REFERENCE_QC <- biocrates %>% filter(Sample.Type == SAMPLE_TYPE_REFERENCE_QC) %>%
  select(Sample.Name) %>% distinct() %>% pull(Sample.Name) %>% length() > 1
```
